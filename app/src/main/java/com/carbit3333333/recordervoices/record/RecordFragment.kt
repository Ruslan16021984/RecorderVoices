...
import com.carbit3333333.recordervoices.MainActivity
...
import com.carbit3333333.recordervoices.databinding.FragmentRecordBinding
...
class RecordFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.playButton.setOnClickListener {
            if (ContextCompat.checkSelfPermission(
                ...
                requestPermissions(
                    arrayOf(Manifest.permission.RECORD_AUDIO), MY_PERMISSION_RECORD_AUDIO
                )
            } else {
                if (mainActivity.isServiceRunning()) {
                    onRecord(false)
    ...
    private fun createChannel(channelIs: String, channelName: String) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val notificationChannel = NotificationChannel(
                channelIs, channelName, NotificationManager.IMPORTANCE_DEFAULT
            ).apply {
                setShowBadge(false)
                setSound(null, null)
